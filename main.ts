...
while (true) {
    ...
    i = 0
    choie = null
    if (spawn_delay != 0) {goalSprite.sayText(target, 20000)}
    while (i < target) {
        chose = Math.pickRandom(possibles)
        console.logValue("gen", chose)
        if (spawn_delay != 0) {playerSprite.sayText(i + 1)}
        goals[i] = chose
        ...
        if (spawn_delay != 0) {pause([0,100,150,200,250][spawn_delay])}
        i += 1
    }
    if (spawn_delay != 0) {pause(250)}
